...
UNITS_COMMON = [(re.compile('%s' % x[0]), x[1]) for x in [
	...
	(r'\b(%s)\s?kmph\b'%(number), r"\1 kilometres per hour"),
	(r'\b(%s)\s?m/hr?\b'%(number), r"\1 miles per hour"),
	(r'\b(%s)\s?mph\b'%(number), r"\1 miles per hour"),
	(r'\b(%s)\s?m/s\b'%(number), r"\1 metres per second"),
	(r'\b(%s)\s?m\b'%(number), r'\1 metres'),
	#(r'\bm\b', r'metres'),
	(r'\b(%s)\s?oz\b'%(number), r'\1 ounces'),
	(r'\b(%s)\s?fl oz\b'%(number), r'\1 fluid ounces'),
	(r'\b(%s)\s?ft\b'%(number), r'\1 feet'),
	(r'\b(%s)\s?gal\b'%(number), r'\1 gallons'),
